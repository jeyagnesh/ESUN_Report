report 50170 "Purchase CreditMemo"
{
        ...
        dataitem("Header"; "Purch. Cr. Memo Hdr.")
        {
            ...
            dataitem(CopyLoop; Integer)
            {
                ...
                dataitem(PageLoop; Integer)
                {
                    ...
                    column(ExternalDocument_No; Header."Vendor Cr. Memo No.")
                    {
                    ...
                    column(SellToNo; Header."Buy-from Vendor No.")
                    {
                    ...
                    column(BillToNo; Header."Pay-to Vendor No.")
                    {
            ...
            begin
                ...
                SellTo[1] := "Buy-from Vendor Name";
                SellTo[2] := "Buy-from Address";
                SellTo[3] := "Buy-from Address 2";
                RecCountry.Reset;
                if "Buy-from Post Code" <> '' then begin
                    if RecCountry.Get("Buy-from Country/Region Code") then
                        SellTo[4] := RecCountry.Name + ' ' + "Buy-from Post Code";
                end;
                CompressArray(SellTo);
                SellTo[5] := "Buy-from Contact";
                if Customer.Get("Buy-from Vendor No.") then begin
                    SellTo[6] := Customer."Phone No.";
                    SellTo[7] := Customer."Fax No.";
                end;

                BillTo[1] := "Pay-to Name";
                BillTo[2] := "Pay-to Address";
                BillTo[3] := "Pay-to Address 2";
                RecCountry.Reset;
                if "Pay-to Post Code" <> '' then begin
                    if RecCountry.Get("Pay-to Country/Region Code") then
                        SellTo[4] := RecCountry.Name + ' ' + "Pay-to Post Code";
                end;
                CompressArray(BillTo);
                BillTo[5] := "Pay-to Contact";
                if Customer.Get("Pay-to Vendor No.") then begin
    ...
    rendering
    {
        ...
        {
            Type = RDLC;
            LayoutFile = './ReportLayout/50170.ReportLayout.PurchCreditMemo.rdl';
        }
    }
    ...
    var
        ...
        GSTRate: Decimal;
        Currency: Code[10];
        Salescrmemo: Record "Purch. Cr. Memo Line";
        ...
        DocTittle: Label 'Purchase Credit Memo';
        NoText: array[2] of Text[80];
        RepCheck: Report Check;
        AmountInWords: Text;
        Amounts: Decimal;
        LblSellToNo: Label 'Buy From';
        LblBillTo: Label 'Pay To';
        LblPostedNo: Label 'Posted No.';
        PageLbl: Label 'Page';
        FormatDocument: Codeunit "Format Document";
        Text004: Label 'PURCHASE CREDIT NOTE %1', Comment = '%1 = Document No.';
