report 50218 "Himetal-Customer Statement"
{
    ApplicationArea = all;
    UsageCategory = ReportsAndAnalysis;
    RDLCLayout = 'Himetal-ReportLayout/50218-Himetal CustomerStatement.rdl';
    WordLayout = 'Himetal-ReportLayout/50218-Himetal CustomerStatement.docx';
    Caption = 'Himetal-Customer Statement';
    DefaultLayout = Word;
    ...
    dataset
    {
        dataitem(Customer; Customer)
        {
            ...
            RequestFilterFields = "No.", "Search Name", "Print Statements", "Currency Filter", "Salesperson Code", "Customer Posting Group";
            column(No_Cust; "No.")
            {
            }
            dataitem("Integer"; "Integer")
            {
                ...
                column(RegNo_CompanyInfo; CompanyInfo."VAT Registration No.") { }
                ...
                column(SwiftCode_CompInfo; CompanyInfo."SWIFT Code") { }
                ...
                column(PostDate_Cap; PostDate_DtldCustLedgEntriesCaptionLbl)
                {
                }
                column(DocNo_Cap; DtldCustLedgEntries.FieldCaption("Document No."))
                {
                }
                ...
                column(RemainAmtCap; 'Credit')
                {
                }
                column(CustBalanceCap; CustBalanceCaptionLbl)
                {
                }
                column(OriginalAmt_Cap; 'Debit')
                {
                }
                ...
                column(RegNoCaptionLbl; RegNoCaptionLbl) { }
                ...
                dataitem(CurrencyLoop; "Integer")
                {
                    ...
                    dataitem(CustLedgEntryHdr; "Integer")
                    {
                        ...
                        dataitem(DtldCustLedgEntries; "Detailed Cust. Ledg. Entry")
                        {
                            ...
                            column(PostDate_Dtld; Format("Posting Date"))
                            {
                            }
                            column(DocNo_Dtld; "Document No.")
                            {
                            }
                            column(Description; Description)
                            {
                            }
                            column(External_Docment_No; External_Docment_No)
                            {
                            ...
                            column(Amt_Dtld; Amount)
                            {
                                AutoFormatExpression = "Currency Code";
                                AutoFormatType = 1;
                            }
                            column(RemainAmt_Dtl; RemainingAmount)
                            {
                                AutoFormatExpression = "Currency Code";
                                AutoFormatType = 1;
                            }
                            ...
                            column(varDeliveryDate; format(varDeliveryDate))
                            {
                            }
                            column(InvoiceAmount; TxtInvoiceAmount)
                            {
                                AutoFormatExpression = "Currency Code";
                                AutoFormatType = 1;
                            }
                            column(PaymentAmount; TxtPaymentAmount)
                            {
                                AutoFormatExpression = "Currency Code";
                                AutoFormatType = 1;
                            }
                            ...
                            var
                                Skip: Boolean;
                                RecSalesInvoiceHdr: Record "Sales Invoice Header";
                            begin
                                ...
                                case "Entry Type" of
                                    "Entry Type"::"Initial Entry":
                                        begin
                                            ...
                                            External_Docment_No := CustLedgerEntry."External Document No.";
                                            Skip := false;
                                            ...
                                            CustLedgerEntry.CalcFields("Remaining Amount", "Credit Amount", "Debit Amount");
                                            RemainingAmount := CustLedgerEntry."Remaining Amount";
                                            CustLedgerEntry.SetRange("Date Filter");

                                            PaymentAmount := 0;
                                            InvoiceAmount := 0;
                                            if (CustLedgerEntry."Debit Amount" > 0) and ("Document Type" <> "Document Type"::Payment) and ("Document Type" <> "Document Type"::"Credit Memo") then begin
                                                InvoiceAmount := CustLedgerEntry."Debit Amount";
                                                RecSalesInvoiceHdr.Reset();
                                                RecSalesInvoiceHdr.SetRange("No.", CustLedgerEntry."Document No.");
                                                if RecSalesInvoiceHdr.FindSet then
                                                    Description := RecSalesInvoiceHdr."Ship-to Address" + ',' + RecSalesInvoiceHdr."Ship-to Address 2"
                                                else
                                                    Description := CustLedgerEntry.Description;
                                            end else begin
                                                PaymentAmount := (CustLedgerEntry."Credit Amount");
                                                Description := CustLedgerEntry.Description;
                                            end;
                                        end;
                                        ...
                                        begin
                                            ...
                                            if CustLedgerEntry.Get("Cust. Ledger Entry No.") then begin
                                                ...
                                                if (CustLedgerEntry."Debit Amount" > 0) then
                                                    ...
                                                    PaymentAmount := (CustLedgerEntry."Credit Amount" * -1);
                                            end;
                                        end;
                            ...
                            "Entry Type"::"Correction of Remaining Amount":
                                ...
                                if InvoiceAmount = 0 then
                                    ...
                                    TxtInvoiceAmount := Format(InvoiceAmount, 0, '<Precision,2:2><Sign><Integer Thousand><Decimals>');

                                if PaymentAmount = 0 then
                                    TxtPaymentAmount := ''
                                else
                                    TxtPaymentAmount := Format(PaymentAmount, 0, '<Precision,2:2><Sign><Integer Thousand><Decimals>');
                    ...
                    {
                        DataItemTableView = SORTING(Number) WHERE(Number = CONST(1));
                        column(CurrencyCode3_Footer; CurrencyCode3)
                        {
                        }
                        column(Total_Caption; Total_CaptionLbl)
                        {
                        }
                        column(CustBalance_Footer; CustBalance)
                        {
                            AutoFormatExpression = TempCurrency2.Code;
                            AutoFormatType = 1;
                        }
                    ...
                    dataitem(OverdueVisible; "Integer")
                    {
                        ...
                        column(PostDate_DtlCaption2; PostDate_DtldCustLedgEntriesCaptionLbl)
                        {
                        }
                        column(DocNo_DtldCaption2; DtldCustLedgEntries.FieldCaption("Document No."))
                        {
                        }
                        column(Desc_Caption2; CustLedgEntry2.FieldCaption(Description))
                        {
                        }
                        column(DueDate_Caption2; DueDate_CustLedgEntry2CaptionLbl)
                        {
                        }
                        column(RemainAmtCaption2; CustLedgEntry2.FieldCaption("Remaining Amount"))
                        {
                        }
                        column(OriginalAmt_Caption2; CustLedgEntry2.FieldCaption("Original Amount"))
                        {
                        }
                        dataitem(CustLedgEntry2; "Cust. Ledger Entry")
                        {
                            ...
                            column(External_Document_No_CustLedgEntry2; "External Document No.") { }
                ...
                dataitem(AgingBandVisible; "Integer")
                {
                    ...
                    dataitem(AgingBandLoop; "Integer")
                    {
                        ...
                        column(AgingBandEndingDate; StrSubstNo('Aged Summary by %1', AgingBandEndingDate, PeriodLength))
                        {
                        }
            ...
            begin
                ...
                //01Mar2022
                CustAddr[1] := Customer."No.";
                CustAddr[2] := Customer.Name;
                CustAddr[3] := Customer.Address;
                CustAddr[4] := Customer."Address 2";
                if RecCountry.get(Customer."Country/Region Code") then
                    CustAddr[5] := Customer.City + ' ' + RecCountry.Name + ' ' + Customer."Post Code"
                else
                    CustAddr[5] := Customer.City + ' ' + Customer."Post Code";
                //01Mar2022
                CompressArray(CustAddr);
                IsFirstLoop := false;

                Clear(PaymentTermDesc);
                PaymentTerms.Reset();
    ...
    var
        ...
        StatementCaptionLbl: Label 'STATEMENT OF ACCOUNT';
        PhoneNo_CompanyInfoCaptionLbl: Label 'Tel:';
        ...
        CompanyInfoHomepageCaptionLbl: Label 'Website:';
        DocDateCaptionLbl: Label 'Document Date';
        FaxCaptionLbl: Label 'Fax:';
        CompanyInfoEmailCaptionLbl: label 'Email:';
        RegNoCaptionLbl: label 'UEN/GST Reg No:';
