report 50229 HimetalPostPurchRecpt
{
    ApplicationArea = All;
    Caption = 'HimetalPostPurchRecpt';
    UsageCategory = ReportsAndAnalysis;
    PreviewMode = PrintLayout;
    DefaultLayout = RDLC;
    RDLCLayout = './Himetal-ReportLayout/50229.ReportLayout.PurchaseReceipt.rdl';
        ...
        dataitem(PurchRcptHeader; "Purch. Rcpt. Header")
        {
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(DocumentDate; Format(PurchRcptHeader."Document Date", 0, '<Day,2>/<Month,2>/<Year>'))
                    {
                    ...
                    column(VendorOrder_No; PurchRcptHeader."Vendor Order No.")
                    {
                    ...
                    // column(ShippingTerms; PurchRcptHeader.Shipment)
                    // {

                    // }
                    column(Purchase; PurchRcptHeader."Purchaser Code")
                    {
                    ...
                    // column(Work_Description; PurchRcptHeader.GetWorkDescription())
                    // {
            ...
            begin
                ...
                PurchRcptLn.Reset();
                PurchRcptLn.SetRange("Document No.", PurchRcptHeader."No.");
                if PurchRcptLn.FindSet() then begin
                    repeat
                        SubTotal += PurchRcptLn.Quantity * PurchRcptLn."Unit Cost";
                        Gst += PurchRcptLn."VAT Base Amount";
                    until PurchRcptLn.Next() = 0;
                    GrandTotal := SubTotal + Gst;
                end;

                PurchRcptLn.Reset();
                PurchRcptLn.SetRange("Document No.", "No.");
                PurchRcptLn.SetFilter("VAT %", '>%1', 0);
                if PurchRcptLn.FindFirst() then
                    GSTRate := PurchRcptLn."VAT %";
                ...
                InitTextVariable;
                FormatNoText(Saying, Round(GrandTotal, 0.01), '');
